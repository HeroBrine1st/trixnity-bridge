...
internal fun Connection.toDriver() = R2dbcDriver(this) {
    it?.printStackTrace()
}

internal suspend fun <T> Publisher<T>.awaitFirst(): T = suspendCancellableCoroutine { continuation ->
...
internal suspend fun <T> Publisher<T>.awaitCompletion() = suspendCancellableCoroutine { continuation ->
 ...
 */
internal suspend fun <T> Publisher<T>.completeAnyway() {
    val job = Job()
...
internal fun <T> Publisher<T>.toFlux(): Flux<T> = Flux.from(this)